...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><div className="shell"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Tools", href: "/#tools" }, { label: page.title }]} /><header className="tool-hero"><span className="eyebrow">{tool.category}</span><h1>{page.title}</h1><p>{page.description}</p></header><AdSlot slot="tool-header" /><section className="workspace" aria-label={page.title}><ToolWorkspace tool={tool} /></section><div className="content-with-ad"><article className="tool-content"><h2>What is {page.title}?</h2><p>{page.intro}</p><h2>How to use this tool</h2><ol>{page.how_to.map((step) => <li key={step}>{step}</li>)}</ol><h2>Why use {page.title}?</h2><p>This focused utility is designed for quick, accessible use across phones, tablets, and desktop browsers. There is no registration wall, and the interface avoids unnecessary steps. Always review important converted documents because complex source formatting can vary between file standards.</p><section className="faq"><h2>Frequently asked questions</h2>{page.faq.map((item) => <details key={item.question}><summary>{item.question}</summary><p>{item.answer}</p></details>)}</section></article><AdSlot slot="tool-sidebar" className="sidebar-ad" /></div></div><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, "\\u003c") }} /></>;
}
