...
async function run(command: string, args: string[], timeoutMs = 120_000) { await new Promise<void>((resolve, reject) => { const child = spawn(command, args, { stdio: ["ignore", "ignore", "pipe"], env: { ...process.env, HOME: tmpdir() }, shell: false }); let stderr = ""; const timer = setTimeout(() => { child.kill("SIGKILL"); reject(new Error("Conversion timed out. Try a smaller or simpler file.")); }, timeoutMs); child.stderr.on("data", (chunk) => { stderr += chunk.toString().slice(0, 2000); }); child.once("error", (error) => { clearTimeout(timer); const spawnError = error as NodeJS.ErrnoException; reject(spawnError.code === "ENOENT" ? new Error(`Required converter '${command}' is not installed on this server.`) : spawnError); }); child.once("close", (code) => { clearTimeout(timer); if (code === 0) resolve(); else reject(new Error(stderr.trim() || `Converter exited with status ${code}.`)); }); }); }
